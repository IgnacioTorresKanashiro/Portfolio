...
if ($link !== false)
{
  ...
  switch ($_GET['accion'])
  {
    ...
    $email = mysqli_real_escape_string($link, $data['email']);
    $password = mysqli_real_escape_string($link, $data['password']);
    $existUser = mysqli_query($link, "SELECT * FROM usuarios WHERE email = '$email' and password = '$password'and type = 2");
    $existAdmin = mysqli_query($link, "SELECT * FROM usuarios WHERE email = '$email' and password = '$password'and type = 1");
    $rowUser = mysqli_num_rows($existUser);
    ...
    if ($rowUser >= 1)
    {
      ...
      header('Status: 200 OK', true, 200);
      $usuarios = [];
      $resultado_usuarios = mysqli_query($link, "SELECT nombre_usuario, type, id, apellido, telefono, localidad, email, foto FROM usuarios where email = '$email'");
