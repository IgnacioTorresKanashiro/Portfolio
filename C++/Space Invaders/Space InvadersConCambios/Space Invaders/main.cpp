...
void Nave::mueve()
{
    ...
    if(tipo==10)
    {
        if(key[KEY_A])  //Para mover la nave se resta o suma sobre el eje X
        {
            x-=5;
        }
...
{
    if(tipo!=0 && tipo!=10) //Permite que solo se creen las balas de los enemigos y que no se repita la de la nave
    {
...
void acomoda_enemigos(Nave E[])
{
    ...
    for(int i=0;i<5;i++) //Las filas de enemigos
    {
        ...
        {
            indice++;
            if(_tipo==1) //Multiplico para hacer espacios iguales entre todos los enemigos
            {
                E[indice].inicia("Recursos/enemigos.bmp", "Recursos/Bala_enem1.bmp", 6, 12, 25, 20, 140+j*30, 130+i*24, 8, _tipo, 1);
            }
            if(_tipo==2) //Multiplico para hacer espacios iguales entre todos los enemigos
            {
                E[indice].inicia("Recursos/enemigos.bmp", "Recursos/Bala_enem2.bmp", 6, 12, 25, 20, 140+j*30, 130+i*24, 8, _tipo, 1);
            }
            if(_tipo==3) //Multiplico para hacer espacios iguales entre todos los enemigos
            {
                E[indice].inicia("Recursos/enemigos.bmp", "Recursos/Bala_enem3.bmp", 6, 12, 25, 20, 140+j*30, 130+i*24, 8, _tipo, 1);
            }
        }
    }
}
...
int main()
{
    ...
    Nave N1, N2;
    N1.inicia("Recursos/nave.bmp", "Recursos/Bala2.bmp", 6, 12, 30, 20, ANCHO/2, ALTO-70, -8, 0, 3);     //Cargo las imágenes a los búfferes, divido por dos para que la nave aparezca en el medio y como las coordenadas están invertidas resto para subir la posición de la nave
    N2.inicia("Recursos/nave2.bmp", "Recursos/Bala2.2.bmp", 6, 12, 30, 20, (ANCHO/2)+50, ALTO-70, -8, 10, 3);
    Nave E[60];
    acomoda_enemigos(E);
    Balas disparos1[N1.max_disp];
    Balas disparos2[N2.max_disp];
    ...
    while(!key[KEY_ESC]) //La rutina del juego se sigue ejecutando mientras no se presione la tecla Escape
    {
        ...
        if(N1.vida!=0)
        {
            crear_bala_nave(N1, disparos1);
        }

        if(N2.vida!=0)
        {
            crear_bala_nave(N2, disparos2);
        }

        if(N1.vida!=0)
        {
            N1.dispara(disparos1, buffer);
        }

        if(N2.vida!=0)
        {
            N2.dispara(disparos2, buffer);
        }


        for(int i=0; i<55; i++) //Recorre por los 55 enemigos
        {
            if(elimina_bala_objeto(N1, E[i], disparos1))//Cuando la bala colisiona con la nave elimina la bala
            {
                explosion1(E[i], buffer); //Si hubo una colisión que imprima una explosión
            }

            if(elimina_bala_objeto(N2, E[i], disparos2))//Cuando la bala colisiona con la nave elimina la bala
            {
                explosion1(E[i], buffer); //Si hubo una colisión que imprima una explosión
            }
        }
        eliminar_bala_escudo(N1,ES,disparos1);
        eliminar_bala_escudo(N2,ES,disparos2);

        pintar_enemigo(E, buffer, mov);
        crear_bala_enemigo(E, azar);
        E[azar].dispara(disp_E, buffer); //El enemigo dispara

        if(elimina_bala_objeto(E[azar], N1, disp_E)) //Chequea si hubo una colisión de la bala con la nave, y de ser así genera el efecto de explosión
        {
            explosion2(N1, buffer, fondo);
        }

        if(elimina_bala_objeto(E[azar], N2, disp_E)) //Chequea si hubo una colisión de la bala con la nave, y de ser así genera el efecto de explosión
        {
            explosion2(N2, buffer, fondo);
        }
